#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# SlientC2 – TCP flood (tcp.c) eklendi, L4 menüsüne tcpflood komutu
        ...
        if result.stdout:
            ...
            for pid in pids:
                try:
                    ...
                    pass
        # C ile yazılmış tcp flood binary
...
def run_c_binary(binary_name, args):
    """Derlenmiş C binary'sini çalıştırır."""
    if not os.path.exists(binary_name):
        # Eğer binary yoksa tcp.c'yi derle
...
def cmd_help():
    ...
    for line in help_lines:
        print(gradient_text(pad + line))
    print(gradient_text(pad + " " * 4 + "Örnek: L7, Layer7, L4, Layer4, udp, httpflood, cfpro, flood, http-raw, http-socket, http-rand, https-spoof, tls, tls2, tls3, httpflood2, slow, tcpflood, saldırı_goster, saldırı_kapan, theme, clear, history, stop\n"))
