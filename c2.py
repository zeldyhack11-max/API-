...
# + GAME menüsü ve FORTNITE metodu eklendi
        ...
        if result.stdout:
            ...
            for pid in pids:
                try:
                    ...
                    pass
        result = subprocess.run(["pgrep", "-f", "udp.py|cf-pro.py|goldeneye.py|https-spoof.py|fortnite.py"], capture_output=True, text=True)
        if result.stdout:
            pids = result.stdout.strip().split('\n')
            for pid in pids:
                try:
...
def cmd_l7():
    ...
    l7_lines = [
        "╔══════════════════════════════════════════════════════════╗",
        "║                    ARTICLES                             ║",
        "║  One of the most-advanced C2/API-Panel on Market!      ║",
        "║  Enjoy <3                                              ║",
        "║  Disorder Channel:                                     ║",
        "║  disorder.gg/articl                                    ║",
        "║  Disorder Owner: Bartico2, ontop                       ║",
        "║  Extra Sections:                                       ║",
        "║    [IV] > star                                         ║",
        "║    [IV] > vip                                          ║",
        "║    [IV] > private                                      ║",
