#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# SlientC2 – Gelişmiş DDoS Aracı (Tüm metodlar + Gradient Prompt)
...
def build_gradient_palette(color1, color2, length):
    ...
    for i in range(length):
        ...
        # 256 renk koduna dönüştür (yaklaşık)
...
# Yeni stil için segmentler ve renkler (gradient kapalıyken kullanılır)
...
# Gradient ayarları
PROMPT_GRADIENT = False          # True ise gradient aktif
PROMPT_GRADIENT_TYPE = None      # "tool" veya ["renk1", "renk2"] veya None
PROMPT_GRADIENT_PALETTE = []     # hesaplanmış palette

# Renk haritası (isim -> Colorama nesnesi) - sadece gradient kapalıyken
COLOR_MAP = {
...
def prompt():
    ...
    if PROMPT_STYLE == 1:
        ...
        # Yeni stil
        if PROMPT_GRADIENT:
            # Gradient aktif: tüm metin tek bir gradient arkaplanla
            ...
            if PROMPT_GRADIENT_TYPE == "tool":
                ...
                palette = [255] * len(full_text)  # fallback
            # Arkaplanı gradient yap, yazı rengini siyah yap (kontrast için)
            ...
            # Gradient kapalı: segmentli renkler
            custom_prompt = (
...
def cmd_promptcolor(args):
    ...
    if PROMPT_STYLE != 2:
        print("[!] Bu komut sadece yeni stil (stil 2) aktifken çalışır.")
        return
    # Gradient aktifse uyarı ver ve kapat
    if PROMPT_GRADIENT:
...
def cmd_promptgradient(args):
    ...
    if args[0].lower() == "off":
        ...
        # Palette'yi hemen hesapla (metin uzunluğu belli değil, prompt() içinde hesaplanacak)
...
def cmd_help():
    ...
    for line in help_lines:
        print(gradient_text(pad + line))
    print(gradient_text(pad + " " * 4 + "Örnek: promptgradient red white, promptgradient tool, promptgradient off\n"))
