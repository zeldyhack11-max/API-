#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# SlientC2 – Gelişmiş DDoS Aracı (8 Tema + Çok Renkli Gradient Prompt + Tüm Metodlar)
...
def build_gradient_palette(colors, length):
    """Birden fazla renk arasında yumuşak geçişli palette oluşturur."""
    if len(colors) < 2:
        return [color_name_to_256(colors[0])] * length
    palette = []
    color_codes = [color_name_to_256(c) for c in colors]
    segment_length = length // (len(colors) - 1)
    for i in range(len(colors) - 1):
        c1 = color_codes[i]
        c2 = color_codes[i + 1]
        r1, g1, b1 = color_name_to_rgb(colors[i])
        r2, g2, b2 = color_name_to_rgb(colors[i + 1])
...
# ========== PROMPT ÖZELLEŞTİRME (ÇOK RENKLİ GRADIENT) ==========
PROMPT_STYLE = 2  # 1 = eski, 2 = yeni

# Yeni stil için segmentler (gradient kapalıyken manuel renkler)
...
PROMPT_GRADIENT_TYPE = None  # "tool" veya ["renk1","renk2","renk3"...] veya None

# Renk haritası (Colorama - manuel renkler için)
COLOR_MAP = {
...
def cmd_promptcolor(args):
    ...
    if len(args) == 0:
        ...
        print("Örnek: promptcolor red white")
        print("        promptcolor red white blue")
        print("        promptcolor tool")
        print("        promptcolor off")
        return
    if args[0].lower() == "off":
        PROMPT_GRADIENT = False
        PROMPT_GRADIENT_TYPE = None
        print("[+] Gradient kapatıldı, manuel renklere dönüldü.")
        return
    if args[0].lower() == "tool":
        ...
        return
    if len(args) < 2:
        print("[!] En az 2 renk belirtin. Örnek: promptcolor red white")
        return
    colors = args[:]
    valid_colors = list(color_name_to_rgb.__code__.co_consts)[1:-1]
    for c in colors:
...
def cmd_promptgradient(args):
    # Eski komut, artık promptcolor ile birleştirildi, yönlendir
    print("[!] Artık 'promptgradient' yerine 'promptcolor' kullanın.")
    print("   Kullanım: promptcolor <renkler>  (örn: promptcolor red white blue)")
    print("             promptcolor tool")
    print("             promptcolor off")
...
def cmd_help():
    ...
    help_lines = [
        ...
        "║  promptcolor - Yeni stil renklerini ayarla (örn: promptcolor red white blue)",
        "║              - promptcolor tool (tool teması) / promptcolor off (kapat)",
    ...
    for line in help_lines:
        print(gradient_text(pad + line))
    print(gradient_text(pad + " " * 4 + "Örnek: promptcolor red white blue, theme 8\n"))
