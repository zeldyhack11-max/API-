#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# SlientC2 – Gelişmiş DDoS Aracı (Tüm metodlar + Özel Prompt)
...
import json
import signal
from colorama import Fore, init, Style, Back  # Back eklendi
...
# ========== PROMPT ==========
# Özel prompt segmentleri (değiştirilebilir)
...
def cmd_setprompt(args):
    global PROMPT_SEGMENT_1, PROMPT_SEGMENT_2
    if len(args) >= 1:
        new_prompt = " ".join(args)
        PROMPT_SEGMENT_1 = f" {new_prompt} "
        print(f"[+] Prompt güncellendi: {PROMPT_SEGMENT_1}{PROMPT_SEGMENT_2}")
    else:
        print("[!] Kullanım: setprompt <yeni_prompt_metni>")
...
def cmd_help():
    ...
    help_lines = [
        ...
        "║  setprompt  - Prompt'u değiştir (örnek: setprompt mybot • attack)",
    ...
    for line in help_lines:
        print(gradient_text(pad + line))
    print(gradient_text(pad + " " * 4 + "Örnek: L7, Layer7, L4, Layer4, GAME, udp, httpflood, cfpro, flood, http-raw, http-socket, http-rand, https-spoof, tls, tls2, tls3, httpflood2, slow, tcpflood, fortnite, fivem, browser, minecraft, dnsamp, ssdp, rapid, uambypass, httpsbypass, setprompt, saldırı_goster, saldırı_kapan, theme, clear, history, stop\n"))
