#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# SlientC2 – Gelişmiş DDoS Aracı (Hiçbir Şey Eksik Değil)
...
def tool_color3():
    idx = len(line_palette) - 1 if line_palette else 231
    return f"\033[38;5;{line_palette[idx]}m"
...
def gradient_text(text):
    """Kutuları bozmayan gradient yazı"""
...
# ========== RENK YARDIMCILARI (Gradient için) ==========
...
def show_logo():
    ...
    print()
    print(gradient_text("    ╔════════════════════════════════════╗"))
    print(gradient_text("    ║        DdoS Attack Tool            ║"))
    print(gradient_text("    ║      Telegram: @SlientBotnet       ║"))
    print(gradient_text("    ╚════════════════════════════════════╝"))
    print()
    print(gradient_text("    ╔════════════════════════════════════╗"))
    print(gradient_text("    ║    write 'help' for usage          ║"))
    print(gradient_text("    ╚════════════════════════════════════╝"))
    print()
...
def cmd_promptcolor(args):
    ...
    if args[0] == "1":
        ...
        c = args[1].lower()
        if c not in valid_colors: return
        PROMPT_COLOR = c
        PROMPT_STYLE = 1
...
def cmd_menu():
    ...
    print(f"{c}║  promptstyle .. <1|2|3>                                  [DEFL]  ║{r}")
    print(f"{c}║  promptcolor .. <...>                                    [DEFL]  ║{r}")
    print(f"{c}║  setprompt .... <metin>                                  [DEFL]  ║{r}")
...
# ========== L7 / L4 / GAME (Menü çağırır) ==========
def cmd_l7(): cmd_menu()
def cmd_l4(): cmd_menu()
def cmd_game(): cmd_menu()
...
def cmd_info(args):
    if len(args) < 1:
        ...
        return
    target = args[0]
    show_attack_banner(target, "N/A", "0", "INFO", "Keşif")
    try:
...
def cmd_udp(args):
    if len(args) < 3:
        ...
        return
    show_attack_banner(args[0], args[1], args[2], "UDP", "2048/16384")
    history_ekle(args[0], args[1], args[2], "UDP", "2048/16384")
...
def cmd_cfpro(args):
    ...
    run_cfpro_script(args[0], args[1], args[2])
...
def cmd_help():
    ...
    print(f"{c}║  promptstyle   - Prompt stili (1|2|3)                    ║{r}")
    print(f"{c}║  promptcolor   - Prompt renkleri (tüm stiller)           ║{r}")
    print(f"{c}║  setprompt     - Prompt metni (stil 2)                   ║{r}")
