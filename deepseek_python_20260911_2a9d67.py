#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# SlientC2 – Gelişmiş DDoS Aracı + info + Düzgün Kutu & Gradient
...
TEMALAR = {
    ...
    8: {"isim": "Kırmızı-Beyaz", "palet": [196, 202, 208, 214, 220, 226, 231]}
}
...
# ========== GRADIENT TEXT (Kutuları Bozmayan) ==========
def gradient_text(text):
    """Kutuları bozmayan düz renk"""
    if not line_palette or len(line_palette) == 0:
        return text
    color = line_palette[0] if line_palette else 196
...
def build_gradient_palette(colors, length):
    """Ultra yumuşak geçişli palette (sağdan sola)"""
    if len(colors) < 2:
...
def show_logo_and_boxes():
    os.system('clear')
    box_color = line_palette[0] if line_palette else 196
    for line in ascii_art:
        print(f"\033[38;5;{box_color}m{pad}{line}\033[0m")
    print(f"\033[38;5;{box_color}m{pad}╔════════════════════════════════════╗\033[0m")
    print(f"\033[38;5;{box_color}m{pad}║        DdoS Attack Tool            ║\033[0m")
    print(f"\033[38;5;{box_color}m{pad}║      Telegram: @SlientBotnet       ║\033[0m")
    print(f"\033[38;5;{box_color}m{pad}╚════════════════════════════════════╝\033[0m")
    print(f"\033[38;5;{box_color}m{small_pad}╔══════════════════════════════╗\033[0m")
    print(f"\033[38;5;{box_color}m{small_pad}║    write 'help' for usage    ║\033[0m")
    print(f"\033[38;5;{box_color}m{small_pad}╚══════════════════════════════╝\033[0m")
    print("\n")
...
def login():
    ...
    box_color = line_palette[0] if line_palette else 196
    print(f"\033[38;5;{box_color}m{pad}╔═════════════════════════════════════╗\033[0m")
    print(f"\033[38;5;{box_color}m{pad}║      Slient DdoS Login              ║\033[0m")
    print(f"\033[38;5;{box_color}m{pad}║   For the password: t.me/Zeldy_here ║\033[0m")
    print(f"\033[38;5;{box_color}m{pad}╚═════════════════════════════════════╝\033[0m")
    ...
    if PROMPT_STYLE == 1:
        ...
        if PROMPT_GRADIENT:
            if PROMPT_GRADIENT_TYPE == "tool":
                ...
                palette = [196] * len(full_text)
            # SAĞDAN SOLA geçiş
            result = ""
            reversed_text = full_text[::-1]
            reversed_palette = palette[::-1]
            for i, char in enumerate(reversed_text):
                color = reversed_palette[i % len(reversed_palette)]
                result = f"\033[48;5;{color}m\033[38;5;16m{char}" + result
...
def show_attack_banner(target, port, duration, method, threads=None, vip=True, expiry=864.70, cooldown=0.00):
    ...
    for line in attacksent_art:
        print(f"\033[38;5;{color}m{pad}{line.center(60)}\033[0m")
    ...
    print(f"\033[38;5;{color}m{pad}╔{border}╗\033[0m")
    print(f"\033[38;5;{color}m{pad}║{left(f'  Target: [{target}]')}║\033[0m")
    print(f"\033[38;5;{color}m{pad}║{left(f'  Time: [{duration}]s')}║\033[0m")
    print(f"\033[38;5;{color}m{pad}║{left(f'  Port: [{port}]')}║\033[0m")
    print(f"\033[38;5;{color}m{pad}║{left(f'  Method: [{method_upper}]')}║\033[0m")
    print(f"\033[38;5;{color}m{pad}╠{'═' * (width - 2)}╣\033[0m")
    if threads is not None:
        print(f"\033[38;5;{color}m{pad}║{left(f'  Threads: [{threads}]')}║\033[0m")
    print(f"\033[38;5;{color}m{pad}║{left(f'  VIP: {vip_str}')}║\033[0m")
    print(f"\033[38;5;{color}m{pad}║{left(f'  Expiry: [{expiry:.2f}]')}║\033[0m")
    print(f"\033[38;5;{color}m{pad}║{left(f'  Cooldown: [{cooldown:.2f}]')}║\033[0m")
    print(f"\033[38;5;{color}m{pad}║{left(f'  Timestamp: [{now}]')}║\033[0m")
    print(f"\033[38;5;{color}m{pad}║{left(f'  Sent by: [{sent_by}]')}║\033[0m")
    print(f"\033[38;5;{color}m{pad}║{empty}║\033[0m")
    print(f"\033[38;5;{color}m{pad}║{left('  Join: t.me/artichetm2')}║\033[0m")
    print(f"\033[38;5;{color}m{pad}║{left('  Discord: @articnetxv')}║\033[0m")
    print(f"\033[38;5;{color}m{pad}╚{border}╝\033[0m")
...
def cmd_l7():
    ...
    for line in l7_lines:
        print(f"\033[38;5;{color}m{pad}{line}\033[0m")
    print(f"\033[38;5;{color}m{pad}    Not: Tüm metodlar gerçektir.\033[0m\n")
...
def cmd_l4():
    ...
    for line in l4_lines:
        print(f"\033[38;5;{color}m{pad}{line}\033[0m")
    print(f"\033[38;5;{color}m{pad}    Not: udp, tcpflood, dnsamp ve ssdp gerçek metodlardır.\033[0m\n")
...
def cmd_game():
    ...
    for line in game_lines:
        print(f"\033[38;5;{color}m{pad}{line}\033[0m")
    print(f"\033[38;5;{color}m{pad}    Not: fortnite, fivem, browser ve minecraft saldırıları mevcuttur.\033[0m\n")
...
def cmd_help():
    ...
    for line in help_lines:
        print(f"\033[38;5;{color}m{pad}{line}\033[0m")
    print(f"\033[38;5;{color}m{pad}    Örnek: info 1.1.1.1, theme 8, promptcolor red white\033[0m\n")
