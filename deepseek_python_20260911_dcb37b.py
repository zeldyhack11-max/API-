#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# SlientC2 – Gelişmiş DDoS Aracı + info + Çoklu Prompt + Renk Seçimi
...
# ========== PROMPT ÖZELLEŞTİRME (3 Stil + Renk Seçimi) ==========
PROMPT_STYLE = 1  # 1 = klasik, 2 = özel (mooncnc), 3 = yeni (└─ $)
PROMPT_SEGMENT_1 = " mooncnc • semp "
PROMPT_SEGMENT_2 = " ▶▶ "

# Manuel renkler (tüm stiller için geçerli)
PROMPT_COLOR_1 = "white"     # Stil 1 ve 3 için yazı rengi
PROMPT_COLOR_2 = "red"       # Stil 2 için ilk segment arkaplanı
PROMPT_COLOR_3 = "white"     # Stil 2 için ikinci segment arkaplanı
PROMPT_COLOR_4 = "black"     # Stil 2 için ilk segment yazı rengi
PROMPT_COLOR_5 = "black"     # Stil 2 için ikinci segment yazı rengi
...
def prompt():
    ...
    # ---- STİL 2: mooncnc • semp ▶▶ (gradientli) ----
    elif PROMPT_STYLE == 2:
...
def cmd_promptstyle(args):
    ...
    if len(args) < 1:
        ...
        print("  1 = Klasik:  ┌──[SlientC2]... / └─➤")
        print("  2 = Özel:    mooncnc • semp ▶▶ (gradient)")
        print("  3 = Yeni:    ┌──[SlientC2]... / └─ $")
        return
    try:
        style = int(args[0])
        if style in (1, 2, 3):
...
def cmd_setprompt(args):
    ...
    if PROMPT_STYLE != 2:
        print("[!] Bu komut sadece stil 2 (mooncnc) aktifken çalışır.")
        return
    if len(args) >= 1:
        new_text = " ".join(args)
...
def cmd_promptcolor(args):
    """Tüm promptlar için renk ayarlama"""
    ...
    if len(args) == 0:
        print("\n[?] Prompt Renk Ayarları")
        print("  Kullanım:")
        print("    promptcolor 1 <renk>                   → Stil 1 yazı rengi")
        print("    promptcolor 2 <bg1> <bg2> <fg1> <fg2>  → Stil 2 segment renkleri")
        print("    promptcolor 3 <renk>                   → Stil 3 yazı rengi")
        print("    promptcolor gradient <r1> <r2> [r3]    → Stil 2 gradient")
        print("    promptcolor tool                       → Stil 2 tool teması")
        print("    promptcolor off                        → Gradient kapat")
        print("\n  Örnek:")
        print("    promptcolor 1 cyan")
        print("    promptcolor 2 red white black black")
        print("    promptcolor 3 magenta")
        print("    promptcolor gradient red white")
        print(f"\n  Renkler: {', '.join(valid_colors)}")
        return
    ...
    if args[0].lower() == "tool":
        ...
        PROMPT_STYLE = 2
        print("[+] Stil 2 gradient -> Tool teması aktif.")
        return

    if args[0].lower() == "gradient":
        if len(args) < 3:
            print("[!] En az 2 renk gerekli. Örnek: promptcolor gradient red white")
            return
        ...
        PROMPT_STYLE = 2
        print(f"[+] Stil 2 gradient -> {', '.join(colors)} aktif.")
        return
    ...
    if args[0] == "2":
        if len(args) < 5:
            ...
            print("    Örnek: promptcolor 2 red white black black")
            return
        for c in args[1:5]:
        ...
        PROMPT_STYLE = 2
        PROMPT_GRADIENT = False
        print(f"[+] Stil 2 renkleri: bg1={PROMPT_COLOR_2}, bg2={PROMPT_COLOR_3}, fg1={PROMPT_COLOR_4}, fg2={PROMPT_COLOR_5}")
        return
    ...
    if args[0] == "3":
        ...
        PROMPT_COLOR_1 = c
        PROMPT_STYLE = 3
        print(f"[+] Stil 3 yazı rengi: {c}")
        return

    print("[!] Geçersiz parametre. 'promptcolor' yazıp yardım al.")
...
def cmd_help():
    ...
    help_lines = [
        ...
        "║  promptstyle - Prompt stilini değiştir (1/2/3)",
        "║  setprompt  - Prompt metnini değiştir (stil 2)",
        "║  promptcolor - Prompt renklerini ayarla (tüm stiller)",
    ...
    for line in help_lines:
        print(gradient_text(pad + line))
    print(gradient_text(pad + " " * 4 + "Örnek: promptstyle 3, promptcolor 3 cyan, theme 8, info 1.1.1.1\n"))
