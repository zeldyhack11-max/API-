...
def gradient_text(text):
    """Kutuları bozmayan, tek renkli gradient görünümlü yazı"""
    if not line_palette or len(line_palette) == 0:
        return text
    # Kutular için tek renk kullan (bozmaz)
...
# ========== METHOD PANELİ (Tool temasıyla uyumlu) ==========
def cmd_menu():
    os.system('clear')
    # Tool temasından renkler
    c_border = line_palette[0] if line_palette else 196
    c_title = line_palette[len(line_palette)//2] if line_palette else 51
    c_vip = line_palette[1] if len(line_palette) > 1 else 196
    c_defl = line_palette[len(line_palette)//3] if line_palette else 46
    
    border = f"\033[38;5;{c_border}m"
    title_color = f"\033[38;5;{c_title}m"
    vip_color = f"\033[38;5;{c_vip}m"
    defl_color = f"\033[38;5;{c_defl}m"
    r = "\033[0m"
...
def cmd_addvip(args):
    ...
    for u in data["vip_users"]:
        if u["username"] == username:
            u["password"] = password
            save_vip_users(data)
            print(f"[+] VIP güncellendi: {username}")
            return
    data["vip_users"].append({"username": username, "password": password})
    save_vip_users(data)
    print(f"[+] VIP eklendi: {username}")
...
def cmd_delvip(args):
    ...
    if len(data["vip_users"]) < original_len:
        save_vip_users(data)
        print(f"[+] VIP silindi: {username}")
    else:
        print(f"[!] Bulunamadı: {username}")
