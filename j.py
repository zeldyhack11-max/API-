#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# SlientC2 – Çıktılar gözükür (run ile), prompt açık kalmaz (bloke olur)
...
# ========== KOMUT İŞLEYİCİLER (ÇIKTILAR GÖZÜKÜR) ==========
def run_script(script_name, args):
    ...
    try:
        subprocess.run(cmd)  # Çıktılar terminale yazılır
    except Exception as e:
...
def run_cfpro_script(url, threads, seconds):
    if not os.path.exists("cf-pro.py"):
        ...
        return
    try:
        subprocess.run(["python3", "cf-pro.py", url, threads, seconds])
    except Exception as e:
        print(f"[!] Beklenmeyen hata: {e}")
